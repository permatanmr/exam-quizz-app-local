...
export default function GenerateTab({ examId }: { examId: string }) {
  ...
  const [difficulty, setDifficulty] = useState<"mudah" | "sedang" | "sulit" | "campuran">(
    "sedang"
  );
  ...
  function updateOptionText(index: number, optionId: string, text: string) {
    setDrafts((prev) => {
      ...
      next[index] = {
        ...next[index],
        options: next[index].options.map((o) => (o.id === optionId ? { ...o, text } : o)),
      };
      return next;
    });
  }
  ...
  async function saveSelected() {
    ...
    try {
      ...
      setSavedMsg(`${data.savedCount} soal berhasil disimpan ke bank soal. Cek tab "Soal".`);
      setDrafts(null);
    } finally {
      setSaving(false);
    }
    ...
    <div>
      <div className="card p-5">
        ...
        <p className="mt-1 text-sm text-muted">
          Masukkan topik dan pengaturan soal, AI akan membuatkan draf soal pilihan ganda yang
          bisa Anda tinjau dan edit sebelum disimpan.
        </p>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            ...
            <div>
              ...
              <select
                className="input"
                value={numOptions}
                onChange={(e) => setNumOptions(Number(e.target.value) as 4 | 5)}
              >
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold">
              Draf Soal ({drafts.filter((d) => d.include).length}/{drafts.length} dipilih)
            </h3>
            <button onClick={saveSelected} disabled={saving} className="btn btn-primary text-sm">
              ...
              <div key={index} className="card p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                      ...
                      {d.options.map((opt) => (
                        <div key={opt.id} className="flex items-center gap-2">
                          <input
                            ...
                            onChange={() => updateDraft(index, { correct_option_id: opt.id })}
                            className="h-4 w-4"
                          />
                    ...
                    {d.explanation && (
                      <p className="mt-2 text-xs text-muted">Penjelasan: {d.explanation}</p>
                    )}
                  </div>
                  <button
